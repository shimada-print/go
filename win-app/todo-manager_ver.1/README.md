...
goでmain.goをコンパイルしたらmain.exeを「todo-manager.exe」(Windowsソフト本体の命名の通例)などとリネームし、[open_windows-start-up-folder](../open_windows-start-up-folder).exeをダブルクリックでWindowsのスタートアップフォルダーを開き、ショートカットを入れておくと、OS起動直後に本アプリも起動するので便利です。
